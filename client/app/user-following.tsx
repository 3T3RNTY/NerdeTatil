...
export default function UserFollowingScreen() {
  ...
  const fetchFollowing = async () => {
    try {
      ...
      setError(null)
      if (targetUserId) {
        const result = await UserService.getFollowing(targetUserId, 1, 100)
        setUsers(result.users)
      }
    } catch (err: any) {
      setError(err?.error || 'Takip listesi yüklenemedi')
    } finally {
      setLoading(false)
    }
  }
      ...
      <PageShell>
        <View style={styles.titleArea}>
          ...
          <Text style={styles.subtitle}>
            {users.length === 0 ? 'Henüz kimseyi takip etmiyorsunuz' : `${users.length} kişi`}
          </Text>
        </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {users.length === 0 ? (
            <Text style={styles.emptyText}>Takip edilen kullanıcı yok</Text>
          ) : (
            users.map((u) => (
