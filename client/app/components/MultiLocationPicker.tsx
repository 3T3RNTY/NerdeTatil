...
const MultiLocationPicker: React.FC<MultiLocationPickerProps> = ({
  ...
  const [visitDates, setVisitDates] = useState<{ [key: number]: string }>({});
          ...
          <ScrollView style={styles.selectedList}>
            {selectedLocations.map((location, index) => (
              <View key={index} style={styles.selectedItem}>
                ...
                {/* Date Input for this location */}
                <View style={styles.dateInputContainer}>
                  <TextInput
                    style={styles.dateInput}
                    placeholder="Ziyaret tarihi (YYYY-MM-DD)"
                    placeholderTextColor="#9ca3af"
                    value={visitDates[index] || ''}
                    onChangeText={(text) => handleSetDate(index, text)}
                  />
                </View>
              </View>
            ))}
